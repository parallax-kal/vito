...
while True:
    ...
    buying_thread = Thread(target=handle_buying, args=(frame,))
    buying_thread.start()
    ...
    for x, y, w, h in faces:
        ...
        c.execute(
            "SELECT customer_name FROM customers WHERE customer_uid LIKE ?",
            (f"{customer_uid}%",),
        )
        row = c.fetchone()
        if row:
        ...
        if 45 < Confidence < 100:
            ...
            cv2.putText(
                frame,
                str(customer_name) + ": " + str(round(Confidence, 2)) + "%",
